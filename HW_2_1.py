...
sj_r = get(sj_url, headers, sj_params, proxies)
sj_path = "sj.rsp"
save_pickle(sj_r, sj_path)
sj_r = load_pickle(sj_path)
...
for v in vacancy_head:
    ...
    if money[0] == 'от':
        ...
        while money[dash + i].isdigit():
            ...
            i += 1
        salary_curr = money[-1]
    vacancy['site'] = 'https://superjob.ru'
    vacancy['name'] = name
    vacancy['link'] = link
...
hh_r = get(hh_url, headers, hh_params, proxies)
hh_path = "hh.rsp"
save_pickle(hh_r, hh_path)
hh_r = load_pickle(hh_path)
soup = bs(hh_r.text, 'html.parser')
vacancy_container = soup.findAll(attrs={"class": "vacancy-serp-item"})
for v in vacancy_container:
    vacancy = {}
