...
def superjob_updater(vacancies_container, collection):
    ...
    for v in vacancy_head:
        ...
        links = []
        for el in vacancies_container:
            links.append(el.get('link'))
        if links.count(link) == 0:
            ...
            print('New vacancy found out', vacancy)
    ...
    for n in is_next:
        if n.text == 'Дальше':
            ...
            superjob_updater(vacancies_container)


def hh_updater(vacancies_container):
    ...
    if len(is_next) != 0 and hh_params['page'] != 3:
        ...
        sleep(randint(100, 200) / 50)
        hh_updater(vacancies_container)


vacancies = []
with open('out.csv', newline='', encoding="utf8") as csv_file:
...
superjob_updater(vacancies)
# hh_updater(vacancies)
